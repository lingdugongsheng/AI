...
class Generator:
    ...
    def evaluate(self, query: str, context: str, answer: str) -> float:
        """
        评估生成回答的质量和置信度。
        改进：使用更明确的评分标准，避免模型返回中间值
        """
        eval_prompt = ChatPromptTemplate.from_messages([
            ("system", """你是一个严格的评估专家。请根据以下标准评估回答质量：

        评分标准（0-1分）：
        - 1.0：回答完全基于上下文，信息准确完整，无任何外部知识
        - 0.8：回答主要基于上下文，少量补充合理
        - 0.6：回答部分基于上下文，但包含明显外部知识
        - 0.4：回答少量基于上下文，主要依赖外部知识
        - 0.2：回答几乎不基于上下文，完全依赖外部知识
        - 0.0：回答与上下文无关或错误

        请仔细检查回答是否严格基于提供的上下文，然后返回一个精确的分数（如 0.85、0.92 等）。
        只返回数字，不要任何解释。"""),
            ("human", """上下文：
        {context}
        ...
        try:
            ...
            if response.replace('.', '', 1).isdigit():
                ...
                if match:
                    ...
                    return min(max(score, 0), 1.0)
                return 0.3  # 默认低分（因为模型没有遵守规则）

        except Exception as e:
            print(f"置信度评估失败: {e}")
            return 0.3  # 解析失败时返回低分
...
class RAGChain:
    ...
    def query(self, question: str) -> Dict[str, str]:
        """
        处理用户查询，返回完整答案，并自动保存对话历史。

        :param question: 用户问题
        :return: 包含答案、来源和置信度的字典
        """
        ...
        initial_state = {
            "query": question,
            "chat_history": self.chat_history,  # 使用实例变量
            "documents": [],
            "context": "",
            "answer": "",
            "sources": [],
